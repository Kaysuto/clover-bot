...
| 🚨 Anti-raid | `/config antiraid seuil\|duree\|age-minimum\|alerte\|deverrouiller\|voir` | Rafale d'arrivées → verrouillage (invitations coupées, vérification au maximum) et alerte staff ; compte trop récent → quarantaine ou expulsion |
...
| 📋 Logs | `/config logs salon\|categorie\|voir` | Arrivées/départs, profils, modération, vocal, salons & rôles, AutoMod — 5 catégories activables, salon dédié possible par catégorie |
...
```
https://discord.com/oauth2/authorize?client_id=<CLIENT_ID>&scope=bot+applications.commands&permissions=420867344
```

> ℹ️ Par rapport à l'invitation d'origine, ce lien ajoute **Gérer le serveur**, nécessaire aux règles AutoMod, et **Voir les logs d'audit**, qui permet aux logs d'indiquer *qui* a expulsé, banni ou supprimé un salon. Sans cette permission le bot journalise quand même l'événement, mais sans son auteur.
